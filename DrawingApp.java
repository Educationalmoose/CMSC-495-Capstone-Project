...
import javax.imageio.ImageIO;public class DrawingApp extends JFrame {
    ...
    private List<Point> currentPath;    public DrawingApp() {
        setTitle("CMSC-495: AI Drawing Recognizer");
        setSize(500, 700);
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        setLayout(new BorderLayout(15, 15));
        getContentPane().setBackground(new Color(240, 242, 245));        // --- 1. THE RESULTS CARD (NORTH) ---
        ...
        probabilityLabel.setFont(new Font("SansSerif", Font.PLAIN, 14));        resultsCard.add(mainPredictionLabel);
        resultsCard.add(probabilityLabel);
        add(resultsCard, BorderLayout.NORTH);        // --- 2. THE DRAWING CANVAS (CENTER) ---
        drawingCanvas = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);                for (List<Point> path : shapes) {
        ...
        drawingCanvas.setBorder(BorderFactory.createLineBorder(new Color(218, 220, 224), 1));        // Mouse Listeners for Drawing
        ...
        });        add(drawingCanvas, BorderLayout.CENTER);        // --- 3. THE BUTTONS (SOUTH) ---
        submitButton = new JButton("Analyze Drawing");
        clearButton = new JButton("Clear Canvas");        submitButton.addActionListener(e -> {
            if (shapes.isEmpty()) {
        ...
        add(buttonPanel, BorderLayout.SOUTH);        ((JPanel)getContentPane()).setBorder(new EmptyBorder(15, 15, 15, 15));
        setLocationRelativeTo(null);
    }    private void performAnalysis() {
        lockUI(true);
        new Thread(() -> {
            try {
                // Capture the JPanel content as an image
                BufferedImage image = new BufferedImage(drawingCanvas.getWidth(), drawingCanvas.getHeight(), BufferedImage.TYPE_INT_RGB);
                Graphics2D g = image.createGraphics();
                drawingCanvas.printAll(g);
                g.dispose();                // Convert to PNG byte array
                ByteArrayOutputStream baos = new ByteArrayOutputStream();
                ImageIO.write(image, "png", baos);
                byte[] imageBytes = baos.toByteArray();                // Send to FakeClient
                String rawResponse = FakeClient.postImageToServer(imageBytes);                // Update UI with the result
                updateUIWithResults(rawResponse);            } catch (Exception ex) {
                updateUIWithError();
            } finally {
                lockUI(false);
            }
